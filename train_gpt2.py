...
class CausalSelfAttention(nn.module):
    ...
    def forward(self, x):
        ...
        # flash attention through pytorch
        y = F.scaled_dot_product_attention(q, k, v, is_causal=True)

        y = y.transpose(1,2).contigous().view(B, T, C)  # re-assemble the outputs side by side
        # output projection
        y = self.c_proj(y)
        return y
...
class MLP(nn.Module):
    def __init__(self, config):
        ...
        self.gelu = nn.Gelu(approxiamte='tanh')
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        pos = self.arange(0, T, dtype=torch.long, device=idx.device)
...
class DataLoaderLite:
    def __init__(self, B, T, process_rank, num_processes, split):
        ...
        # at initialization load token from disk and store them in memory
    ...
    def next_batch(self):
        ...
        # if loading the next bacth would be out of bounds, advance to next shard
...
# simple launch for hellaswag eval
...
torch.set_float32_matual_precision('high')
...
for step in range(max_steps):
    ...
    if master_process:
        print(f"step{step:4d} | loss: {loss_accum.item():.6f} | lr: {lr:.4e} | norm: {norm:.4f} | dt: {dt*1000}ms| tok/sec: {tokens_per_second: .2f}")
if ddp:
